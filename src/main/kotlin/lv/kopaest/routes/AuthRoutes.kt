...
import lv.kopaest.data.responses.ErrorResponse
    ...
    post("signup") {
        val request = try {
            call.receiveOrNull<RegistrationRequest>() ?: kotlin.run {
                call.respond(HttpStatusCode.BadRequest, ErrorResponse("Something is wrong with the request"))
                return@post
            }
        } catch (exception: SerializationException) {
            call.respond(HttpStatusCode.BadRequest, ErrorResponse("Request body does not contain all required fields"))
            return@post
        }

        if (userDataSource.getUserByEmail(request.email) != null) {
            call.respond(HttpStatusCode.BadRequest, ErrorResponse("User with this email already exists"))
            return@post
        }
        ...
        if (userDataSource.registerUser(user)) {
            call.respond(HttpStatusCode.OK, ErrorResponse("User has been registered successfully!"))
        } else {
            call.respond(HttpStatusCode.Conflict, ErrorResponse("An unknown error occurred"))
        }
    }
}
...
fun Route.signIn(userDataSource: UserDataSource, hashingService: HashingService) {
    post("signin") {
        ...
        if (user == null) {
            call.respond(HttpStatusCode.BadRequest, ErrorResponse("Incorrect username or password"))
            return@post
        }
